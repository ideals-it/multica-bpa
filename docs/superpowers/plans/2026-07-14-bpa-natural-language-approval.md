...
- [ ] **Step 4: Keep reactions non-semantic**
...
- [ ] **Step 1: Write failing instruction and guard tests**
...
- [ ] **Step 2: Implement the instruction contract**
...
- [ ] **Step 3: Preserve scope protection**
...
- [ ] **Step 1: Rebase selected simple-task commits onto `bpa/main`**
...
Run: `make migrate-up && cd server && go test ./internal/approval ./internal/handler ./internal/service ./internal/daemon/execenv ./cmd/server -count=1`

- [ ] **Step 3: Final review and draft PR**

Run three review passes for the integrated diff. Push only the rebased integration branch and create a draft PR into `bpa/main`; do not deploy or restart the real local Multica before merge.
